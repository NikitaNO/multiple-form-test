...
const renderDate = ({ input, label, type, placeholder, meta: { touched, error } }) => {
    ...
    if (val == "YYYY") {
        ...
        return (
          <div className="field-wrapper-date">
              <input maxLength={2} onKeyPress={(event) => {return (event.charCode == 8 || event.charCode == 0 || event.charCode == 45 || event.target.value.length > 1) ? event.preventDefault() : event.charCode >= 48 && event.charCode <= 57}} name="itemConsumption"  {...input} type={type} placeholder={placeholder} min="0"/>
          </div>
        )
    }
};
